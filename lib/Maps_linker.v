...
Module PTree <: TREE.
  ...
  Fixpoint xoption_map (A B : Type) (f : positive -> A -> option B) (m : t A) (i : positive)
    ...
    match m with
      ...
      | Node l o r =>
        Node
          (xoption_map f l (xO i))
          (match o with
             | None => None
             | Some x => f (prev i) x
           end)
          (xoption_map f r (xI i))
    end.

  Definition option_map (A B : Type) (f : positive -> A -> option B) m := xoption_map f m xH.

  Lemma xgoption_map:
    forall (A B: Type) (f: positive -> A -> option B) (i j : positive) (m: t A),
      get i (xoption_map f m j) = match get i m with
                                    | None => None
                                    | Some x => f (prev (prev_append i j)) x
                                  end.
  Proof.
    induction i; intros; destruct m; simpl; auto.
  Qed.
  ...
  Theorem goption_map:
    forall (A B: Type) (f: positive -> A -> option B) (i: positive) (m: t A),
      get i (option_map f m) = match get i m with
                                 | None => None
                                 | Some x => f i x
                               end.
  Proof.
